...
def analyserContenu(domaine: str, verbose: bool = False) -> int:
    ...
    # Analyse des mots-clés dans le texte et les meta-tags
        ...
        for css_class in element["class"]:
            for keyword in CSS_PARKING_KEYWORDS:
                if keyword in css_class:
                    if verbose: print(f"  [+] Classe CSS de parking trouvée ('{css_class}') : +10 pts")
                    score += 10
                    found_css_keyword = True
                    break
            if found_css_keyword: break
        if found_css_keyword: break
    if not found_css_keyword and verbose: print("  [-] Aucune classe CSS de parking trouvée.")
            ...
            if src_domain:
                for parking_host in KNOWN_PARKING_HOSTNAMES:
                    if src_domain.endswith(parking_host):
                        if verbose: print(f"  [+] Source de script/iframe de parking trouvée ('{src_domain}') : +15 pts")
                        score += 15
    ...
    if not found_script_source and verbose: print("  [-] Aucune source de script/iframe de parking trouvée.")

    return min(score, 40) # On plafonne le score de contenu à 40
