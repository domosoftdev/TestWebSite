...
def analyserContenu(domaine: str, verbose: bool = False) -> int:
    ...
    # Vérification de redirection OU de contenu mentionnant un service connu
        ...
        for record in ns_records:
            ...
            for known_ns in KNOWN_PARKING_NAMESERVERS:
                if ns_str.startswith(known_ns.rstrip('.')) or ns_str.endswith(known_ns + '.'):
                    ...
                    return 15 # Signal très fort, on retourne immédiatement
...
def calculerScoreParking(domaine: str, verbose: bool = False) -> int:
    ...
    if score_contenu >= 20: # Early exit for strong content signal
        if verbose: print("\n[i] Signal de contenu fort détecté. Score final basé sur le contenu.")
        return score_contenu

    score_technique = analyserTechnique(domaine, verbose=verbose)
    if score_technique >= 15: # Early exit for strong technical signal
        if verbose: print("\n[i] Signal technique fort détecté. Score final basé sur la technique.")
