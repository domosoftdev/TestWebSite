...
def generate_summary_html(all_scans):
    """
    Génère un rapport de synthèse HTML à partir de tous les scans.
    Le rapport inclut un tableau triable des résultats.
    """
    if not all_scans:
        print("Aucun scan à traiter pour le rapport HTML.")
        return

    # Obtenir uniquement le scan le plus récent pour chaque domaine
    latest_scans = {}
    for scan in sorted(all_scans, key=lambda x: x['date'], reverse=True):
        if scan['domain'] not in latest_scans:
            latest_scans[scan['domain']] = scan

    scans_to_report = sorted(latest_scans.values(), key=lambda x: x['domain'])
...
<head>
    ...
    <style>
        ...
        .grade-A, .grade-A-plus { background-color: #c8e6c9; }
        .grade-B { background-color: #fff9c4; }
        .grade-C { background-color: #ffccbc; }
        .grade-D { background-color: #ffcdd2; }
        .grade-F { background-color: #f8bbd0; font-weight: bold; }
    </style>
</head>
<body>
    <h1>Rapport de Synthèse de Sécurité</h1>
    <p>Ci-dessous, un résumé des derniers scans pour chaque domaine. Cliquez sur les en-têtes pour trier les colonnes.</p>
...
"""
    ...
    <script>
        ...
        document.querySelectorAll("#summaryTable th").forEach(headerCell => {
...
def main():
    ...
    parser.add_argument("--summary-html", action="store_true", help="Génère un rapport de synthèse HTML (summary_report.html) avec des colonnes triables.")
    ...
    if args.list_scans:
        ...
        generate_summary_html(all_scans)
    else:
